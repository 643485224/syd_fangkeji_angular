...
export interface InvsCardResponse {
  result: number;
  sw1: number;
  sw2: number;
  sw3: number;
...
export enum TaskCode {
  ...
  TASK_GET_HW_INFO = 120,
  TASK_EXPORT_RECORD = 122,
  TASK_SET_ID_PAUSE = 123,
  TASK_CARD_OPERATION_INVS = 124,
  TASK_TEMPERATURE_MEASUREMENT = 135,
  TASK_START_ACT_FACE = 201,
  TASK_START_OCR = 301,
  TASK_READ_PP = 302,
  TASK_AES_ENCRYPT = 401,
  TASK_AES_DECRYPT = 402,
  TASK_COMMONLOGO = 501,
  TASK_REBOOT = 801
}
...
export enum EventCode {
  EVENT_FIND_FACE = 1,
  EVENT_SCAN_CODE = 2,
  EVENT_SCAN_TEMPERATURE_MEASUREMENT = 4
}
...
export class NativeApiProvider {
  ...
  public printForm(request: PrintRequest): Promise<TaskResult> {
    let style: number = 0;
    let size: number = 0;
    ...
    try {
      ...
      size = Number(this.getConfig("qrcode_size"));
    } catch (err) {
      console.error(err);
    }

    request.QrcodeSize = size;
  ...
  public async GetTemperature(): Promise<string> {
    ...
    try {
      result = await this.execTask(TaskCode.TASK_TEMPERATURE_MEASUREMENT, 0);
  ...
  public async SetIdReaderPause(pause: boolean): Promise<void> {
    if (!window.$native) return null;

    let result: TaskResult;

    try {
      result = await this.execTask(TaskCode.TASK_SET_ID_PAUSE, pause ? 1 : 0);
    } catch (err) {
      console.error(err);
    }
  ...
  public async InvsCardOperation(data: number, param1?: string, param2?: string, timeout?: number): Promise<InvsCardResponse | null> {
    if (!window.$native) return null;
    ...
    try {
      ...
      result = await this.execTask(TaskCode.TASK_CARD_OPERATION_INVS, data, param1, param2, timeout);
  ...
  public async ICO_FindCard_REQA(timeout?: number): Promise<InvsCardResponse | null> {
    if (!window.$native) return null;
    ...
    try {
      ...
      result = await this.execTask(TaskCode.TASK_CARD_OPERATION_INVS, 1, null, null, timeout);
  ...
  public async ICO_FindCard_WUPA(timeout?: number): Promise<InvsCardResponse | null> {
    if (!window.$native) return null;
    ...
    try {
      ...
      result = await this.execTask(TaskCode.TASK_CARD_OPERATION_INVS, 2, null, null, timeout);
  ...
  public async ICO_VerifyKeyA(sn: number, key: number[], timeout?: number): Promise<InvsCardResponse | null> {
    if (!window.$native) return null;

    let result: TaskResult;

    try {
      let param1: any = new Object();
      ...
      result = await this.execTask(TaskCode.TASK_CARD_OPERATION_INVS, 3, JSON.stringify(param1), null, timeout);
  ...
  public async ICO_VerifyKeyB(sn: number, key: number[], timeout?: number): Promise<InvsCardResponse | null> {
    if (!window.$native) return null;
    ...
    try {
      ...
      result = await this.execTask(TaskCode.TASK_CARD_OPERATION_INVS, 4, JSON.stringify(param1), null, timeout);
  ...
  public async ICO_ReadBlock(bn: number, timeout?: number): Promise<InvsCardResponse | null> {
    if (!window.$native) return null;
    ...
    try {
      ...
      result = await this.execTask(TaskCode.TASK_CARD_OPERATION_INVS, 5, JSON.stringify(param1), null, timeout);
  ...
  public async ICO_WriteBlock(bn: number, data: number[], timeout?: number): Promise<InvsCardResponse | null> {
    if (!window.$native) return null;
    ...
    try {
      ...
      result = await this.execTask(TaskCode.TASK_CARD_OPERATION_INVS, 6, JSON.stringify(param1), null, timeout);
  ...
  public async ICO_WriteCtrlBlock(data: number[], timeout?: number): Promise<InvsCardResponse | null> {
    if (!window.$native) return null;
    ...
    try {
      ...
      result = await this.execTask(TaskCode.TASK_CARD_OPERATION_INVS, 7, JSON.stringify(param1), null, timeout);
